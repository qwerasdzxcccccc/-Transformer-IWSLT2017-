...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1, rel_pos=None):
        super().__init__()
        ...
        self.dropout = nn.Dropout(dropout)
        # rel_pos not used in encoder in our setup, but could be passed
...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1, rel_pos=None):
        ...
        self.dropout = nn.Dropout(dropout)
        self.rel_pos = rel_pos  # relative bias for self-attention & optionally cross-attention
    ...
    def forward(self, x, enc_out, tgt_mask=None, memory_mask=None):
        ...
        rel_bias_cross = None
        # For cross-attention we can choose not to add relative bias (or create another table)
        # here we do NOT add rel-bias for cross-attn (simpler)
...
class TransformerEncDec(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, d_model=256, num_layers=6, num_heads=8, d_ff=1024,
                 max_len=256, dropout=0.1, rel_pos_max_distance=128):
        super().__init__()
        self.src_embed = nn.Embedding(src_vocab_size, d_model)
        self.tgt_embed = nn.Embedding(tgt_vocab_size, d_model)
        self.pos_scale = math.sqrt(d_model)
        self.pos_enc = nn.Parameter(torch.zeros(1, max_len, d_model))  # learned absolute fallback (optional)
        nn.init.normal_(self.pos_enc, std=0.02)
        self.rel_pos = RelativePositionBias(num_heads, max_distance=rel_pos_max_distance)
        # encoder/decoder stacks
        self.encoder_layers = nn.ModuleList([EncoderLayer(d_model, num_heads, d_ff, dropout, rel_pos=self.rel_pos)
                                             for _ in range(num_layers)])
        self.decoder_layers = nn.ModuleList([DecoderLayer(d_model, num_heads, d_ff, dropout, rel_pos=self.rel_pos)
                                             for _ in range(num_layers)])
    ...
    def forward(self, src_ids, tgt_ids, src_mask=None, tgt_mask=None, memory_mask=None):
        ...
        # convert pad masks to attention masks: expects mask shape (batch, 1, 1, seq) maybe,
        # but our attention expects (batch, heads, qlen, klen) fill mask; we'll convert in scaled attention if needed.
        # For simplicity we create attn masks of shape (batch, 1, 1, seq) where 1 indicates valid.
        # However our scaled_dot_product_attention expects mask where valid=1 else 0 and is broadcastable.
        # We'll implement simple broadcast: create mask (batch, 1, 1, klen)
        ...
        # expand to (batch, 1, qlen, klen)
        tgt_attn_mask = causal.unsqueeze(0).unsqueeze(0)
        if tgt_mask is not None:
            # combine padding mask: tgt_mask shape (batch, tgt_len)
            pad_mask = tgt_mask.unsqueeze(1).unsqueeze(2)  # (batch,1,1,tgt_len)
            tgt_attn_mask = tgt_attn_mask & pad_mask  # broadcast: results shape (batch,1,tgt_len,tgt_len) via broadcasting of pad_mask
            # but shapes differ; more robust approach: create mask per batch below in decoder loop
        dec_out = tgt
        dec_attns = []
        cross_attns = []
        # We'll compute a per-batch tgt_attn_mask inside loop to include padding; for simplicity we pass None to attention and let attention handle causal via rel_bias?
        # Simpler: create combined mask of shape (batch, 1, tgt_len, tgt_len) that has True for allowed positions
        if tgt_mask is not None:
