...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        out, attn = scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, dropout=self.dropout)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, src_mask=None):
        # x: (batch, seq, d_model)
        attn_out, attn = self.self_attn(x, x, x, attn_mask=src_mask)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, enc_out, tgt_mask=None, memory_mask=None):
        # masked self-attn
        self_attn_out, self_attn = self.self_attn(x, x, x, attn_mask=tgt_mask)
        x = x + self.dropout(self_attn_out)
        x = self.norm1(x)

        # cross attention (query from decoder, key/value from encoder)
        cross_attn_out, cross_attn = self.cross_attn(x, enc_out, enc_out, attn_mask=memory_mask)
...
# -------------------------
# Full Encoder-Decoder (without relative position bias)
# -------------------------
class TransformerEncDec(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, d_model=256, num_layers=6, num_heads=8, d_ff=1024,
        ...
        # Use only learned absolute position embeddings, without relative position bias
        self.pos_enc = nn.Parameter(torch.zeros(1, max_len, d_model))
        nn.init.normal_(self.pos_enc, std=0.02)
        
        # encoder/decoder stacks without relative position bias
